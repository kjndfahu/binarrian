...
export function MarketTradesSection(){
    ...
    return (
        <div
            className="flex flex-col w-full relative overflow-hidden gap-3 flex-1 rounded-[12px] border-[1px] border-white pt-6 pb-0">
            ...
            <div className="flex flex-col px-6 gap-4 flex-1 min-h-0 overflow-hidden">
                ...
                <div style={{scrollbarWidth: 'thin', scrollbarColor: '#D0DCFF8F transparent'}} className="flex flex-col gap-4 overflow-y-auto">
                    {tradesData.map((item) => (
                        <div className="flex items-center justify-between">
                            <div className="flex-1 font-bold lg:text-[16px] text-[14px] leading-6 text-white">{item.time}</div>
                            <div
                                className={`flex-1 font-bold lg:text-[16px] text-[14px] leading-6 ${item.isPositive ? 'text-[#11CABE]' : 'text-[#FA2256]'}`}>{item.price}
                            </div>
                            <div
                                className="flex-1 font-bold lg:text-[16px] text-[14px] leading-6 text-white">{item.amount}
                            </div>
                            <div
                                className="flex-1 font-bold lg:text-[16px] text-[14px] leading-6 text-white">{item.total}
                            </div>
                        </div>
                    ))}
                </div>
